...
def decode_2(them: str, me: str) -> int:
    t = DECODE_THEM[them]
    match (t, me):
        case (Hand.ROCK, Code.DRAW) | (Hand.PAPER, Code.LOSE) | (Hand.SCISSORS, Code.WIN):
            return Hand.ROCK
        case (Hand.PAPER, Code.DRAW) | (Hand.SCISSORS, Code.LOSE) | (Hand.ROCK, Code.WIN):
            return Hand.PAPER
        case (Hand.SCISSORS, Code.DRAW) | (Hand.ROCK, Code.LOSE) | (Hand.PAPER, Code.WIN):
...
def test_p1():
    assert 15 == parse_run(p1, inputs="""A Y
B X
C Z""")


def test_p2():
    assert 12 == parse_run(p2, inputs="""A Y
B X
C Z""")